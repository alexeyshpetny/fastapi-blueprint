...
class CacheSettings(BaseSettings):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.CACHE_ENABLED and not self.CACHE_PASSWORD:
            raise ValueError(
                "CACHE_ENABLED=True but CACHE_PASSWORD is not set. "
                "For production, it's recommended to set a Redis password "
                "for security. Store the password in a secret management "
                "service (AWS Secrets Manager, HashiCorp Vault, etc.)."
            )
